...
\item \texttt{notFoundPage FILE} sets a custom body for the built-in 404 \texttt{Not Found} page, served (as \texttt{text/html}) for any URL that matches no page handler.  \texttt{FILE} names a file (resolved relative to the \texttt{.urp}'s directory) whose entire contents become the HTML body, so it may span multiple lines and contain any characters (including \texttt{\#}).  The default is a plain-text \texttt{Not Found}.  This styles the one error page not reachable through \texttt{onError} (which handles fatal 500s).
...
\item \texttt{-dbms [postgres|mysql|sqlite]}: Sets the database backend to use.
  \begin{itemize}
  \item \texttt{postgres}: This is PostgreSQL, the default.  Among the supported engines, Postgres best matches the design philosophy behind Ur, with a focus on consistent views of data, even in the face of much concurrency.  Different database engines have different quirks of SQL syntax.  Ur/Web tends to use Postgres idioms where there are choices to be made, though the compiler translates SQL as needed to support other backends.  Deployment note: create the database with \texttt{LC\_COLLATE=C} (or \texttt{POSIX}).  Ur/Web compares strings bytewise (\texttt{strcmp}), while SQL \texttt{ORDER BY} and text comparisons use the database collation; under any other collation (e.g.\ \texttt{en\_US.UTF-8}) the two sort orders diverge silently.  Compiled applications check the database's default collation at startup, next to the schema checks -- provider-aware on PostgreSQL 15+ (libc, ICU, and the builtin provider) -- and print a warning unless the governing locale is bytewise (libc \texttt{C}/\texttt{POSIX}/\texttt{C.utf8}, or the builtin provider's \texttt{C}/\texttt{C.UTF-8}; ICU locales always warn).
...
\item \texttt{-ccopt FLAGS}: Optimization flags passed to the C compiler for the generated code.  The default is \texttt{-O3}.  Because the whole application is emitted as a single large C file, \texttt{gcc -O3} dominates compile time for big applications, so lowering this (e.g. \texttt{-ccopt -O1}) makes iterative and CI builds much faster at the cost of runtime performance; keep \texttt{-O3} for production.  Use \texttt{-O1} rather than \texttt{-O0} as the ``fast'' level: at \texttt{-O0}, glibc's \texttt{\_FORTIFY\_SOURCE} hardening (enabled by default on many distributions and in sandboxed builds) emits a ``requires compiling with optimization'' warning that Ur/Web's \texttt{-Werror} turns into a fatal error.  (\texttt{-debug} forces no optimization regardless.)
\item \texttt{-staticRuntime}: Link the Ur/Web runtime (\texttt{liburweb}) statically while leaving system libraries (\texttt{libc}, the database client, OpenSSL, ICU) dynamic.  This produces a portable executable that carries the runtime without the deployment fragility of a fully static \texttt{-static} binary (which statically links glibc, and with it NSS/\texttt{dlopen}).  It requires the static archive \texttt{liburweb.a} to be installed (present unless the build was configured with \texttt{-{}-disable-static}).  If both \texttt{-static} and \texttt{-staticRuntime} are given, \texttt{-static} takes precedence.
...
A note on \emph{schema migrations} (upstream issue \#65): Ur/Web deliberately has no built-in migration engine.  The emitted SQL script (\texttt{-sql}/the \texttt{sql} directive) is a fresh-install schema, and the startup checks verify -- but never alter -- a live database.  The workflow that works well with standard tooling, proposed and validated in that issue's thread: load the newly emitted script into a \emph{scratch} database, then diff the live database against it with a schema-diff tool (e.g.\ \texttt{migra} for PostgreSQL, which emits the \texttt{ALTER} statements to review and apply).  Because Ur/Web's startup schema check is strict, a successfully restarted application is itself confirmation the migration matched the compiled expectations.
...
The guarded type $[c \sim c] \Rightarrow \tau$ may equivalently be written $[c \sim c] \to \tau$, for consistency with the $\to$ used elsewhere to write the type of a $\Rightarrow$-abstraction.
...
One known incompleteness deserves a note (upstream issue \#266): a directly recursive polymorphic function, exported through a signature with implicit (\cd{:::}) constructor parameters, can fail with ``Substitution in constructor is blocked by a too-deep unification variable'' -- whether or not the recursive call re-passes the constructor parameters explicitly.  Two workarounds, both verified against that issue's program: perform the recursion in a \emph{monomorphic inner helper} (a \cd{let fun go ...} mentioning only the enclosing function's type parameters), which elaborates fine under the same signature; or, for module-private helpers, drop the function from the \cd{.urs} signature and annotate its definition, since the failure arises in matching the recursive definition against the ascribed signature.  The one-line relaxation of the elaborator's depth guard proposed upstream (PR \#226) is deliberately not adopted in this fork: that guard stops substitution at a unification variable's scope boundary, and relaxing it risks scope-escaping substitutions.
...
$$\begin{array}{l}
  ...
  \mt{val} \; \mt{setCookie} : \mt{t} ::: \mt{Type} \to \mt{http\_cookie} \; \mt{t} \to \{\mt{Value} : \mt{t}, \mt{Expires} : \mt{option} \; \mt{time}, \mt{Secure} : \mt{bool}, \mt{HttpOnly} : \mt{bool}\} \to \mt{transaction} \; \mt{unit} \\
...
$$\begin{array}{l}
  ...
  \hspace{.1in} \Rightarrow \{\mt{Rows} : \mt{sql\_query1} \; \mt{free} \; \mt{afree} \; \mt{tables} \; \mt{grouped} \; \mt{selectedFields} \; \mt{selectedExps}, \\
  \hspace{.2in} \mt{OrderBy} : \mt{sql\_order\_by} \; (\mt{free} \rc \mt{grouped}) \; (\mt{afree} \rc \mt{tables}) \; \mt{selectedExps}, \\
...
Most of the complexity of the query encoding is in the type $\mt{sql\_query1}$, which includes simple queries and derived queries based on relational operators.  Constructor arguments respectively specify the unrestricted free table veriables, the aggregate-only free table variables, the tables we select from, the grouped tables (the \texttt{GROUP BY} subset, against which \texttt{ORDER BY} bare columns are checked since the \#269 fix; equal to all tables when there is no \texttt{GROUP BY}), the subset of fields that we keep from each table for the result rows, and the extra expressions that we select.
$$\begin{array}{l}
  \mt{con} \; \mt{sql\_query1} :: \{\{\mt{Type}\}\} \to \{\{\mt{Type}\}\} \to \{\{\mt{Type}\}\} \to \{\{\mt{Type}\}\} \to \{\{\mt{Type}\}\} \to \{\mt{Type}\} \to \mt{Type} \\
  \\
  ...
  \hspace{.1in} \to \mt{grouped1} ::: \{\{\mt{Type}\}\} \\
  \hspace{.1in} \to \mt{grouped2} ::: \{\{\mt{Type}\}\} \\
  \hspace{.1in} \to \mt{selectedFields} ::: \{\{\mt{Type}\}\} \\
  \hspace{.1in} \to \mt{selectedExps} ::: \{\mt{Type}\} \\
  \hspace{.1in} \to \mt{sql\_relop} \\
  \hspace{.1in} \to \mt{bool} \; (* \; \mt{ALL} \; *) \\
  \hspace{.1in} \to \mt{sql\_query1} \; \mt{free} \; \mt{afree} \; \mt{tables1} \; \mt{grouped1} \; \mt{selectedFields} \; \mt{selectedExps} \\
  \hspace{.1in} \to \mt{sql\_query1} \; \mt{free} \; \mt{afree} \; \mt{tables2} \; \mt{grouped2} \; \mt{selectedFields} \; \mt{selectedExps} \\
  \hspace{.1in} \to \mt{sql\_query1} \; \mt{free} \; \mt{afree} \; [] \; [] \; \mt{selectedFields} \; \mt{selectedExps}
\end{array}$$
...
$$\begin{array}{l}
  ...
  \hspace{.1in} \to \mt{sql\_query1} \; \mt{free} \; \mt{afree} \; \mt{tables} \; \mt{grouped} \; \mt{selectedFields} \; \mt{selectedExps}
\end{array}$$
...
$$\begin{array}{l}
  ...
  \\
  \mt{con} \; \mt{sql\_order\_by} :: \{\{\mt{Type}\}\} \to \{\{\mt{Type}\}\} \to \{\mt{Type}\} \to \mt{Type} \\
  \mt{val} \; \mt{sql\_order\_by\_Nil} : \mt{tables} ::: \{\{\mt{Type}\}\} \to \mt{agg} ::: \{\{\mt{Type}\}\} \to \mt{exps} :: \{\mt{Type}\} \to \mt{sql\_order\_by} \; \mt{tables} \; \mt{agg} \; \mt{exps} \\
  ...
  \hspace{.1in} \to \mt{sql\_window} \; \mt{tf} \to \mt{tf} \; \mt{tables} \; \mt{agg} \; \mt{exps} \; \mt{t} \to \mt{sql\_direction} \to \mt{sql\_order\_by} \; \mt{tables} \; \mt{agg} \; \mt{exps} \to \mt{sql\_order\_by} \; \mt{tables} \; \mt{agg} \; \mt{exps} \\
  \mt{val} \; \mt{sql\_order\_by\_random} : \mt{tables} ::: \{\{\mt{Type}\}\} \to \mt{agg} ::: \{\{\mt{Type}\}\} \to \mt{exps} ::: \{\mt{Type}\} \to \mt{sql\_order\_by} \; \mt{tables} \; \mt{agg} \; \mt{exps} \\
  \\
...
$$\begin{array}{l}
  ...
  \hspace{.1in} \to \mt{sql\_partition} \; \mt{tables} \; \mt{agg} \; \mt{exps} \\
  \hspace{.1in} \to \mt{sql\_order\_by} \; \mt{tables} \; \mt{agg} \; \mt{exps} \\
  \hspace{.1in} \to \mt{sql\_expw} \; \mt{tables} \; \mt{agg} \; \mt{exps} \; \mt{t} \\
  \\
...
\item \texttt{ffi FILE} names the \texttt{.urs} file giving your library's signature.  You can include multiple such files in a single \texttt{.urp} file, and each \texttt{ffi mod} defines an FFI module \texttt{Mod}.
...
int uw_register_transactional(uw_context, void *data, uw_callback commit,
  ...
  \end{verbatim}
  All side effects in Ur/Web programs need to be compatible with transactions, such that any set of actions can be undone at any time.  Thus, you should not perform actions with non-local side effects directly; instead, register handlers to be called when the current transaction is committed or rolled back.  The arguments here give an arbitary piece of data to be passed to callbacks, a function to call on commit, a function to call on rollback, and a \texttt{free} function to clean up any allocated resources.  The \texttt{free} callback is the unconditional cleanup hook: under the standard request drivers it is called exactly once per registered transactional per transaction attempt --- after commit, after rollback, or when \emph{neither} of the other two callbacks has run for this transactional, as when an earlier failure or a serialization-failure retry preempts its phase.  (The standard drivers reset the per-request transactional list between attempts; embedders driving the C API directly must do the same, since calling \texttt{uw\_rollback()} after a failed \texttt{uw\_commit()} without such a reset can invoke \texttt{rollback} and \texttt{free} a second time.)  The \texttt{commit} and \texttt{rollback} callbacks are each conditional: a \texttt{commit} callback may be skipped entirely, and a rollback handler may be called after the associated commit handler has already been called, if some later part of the commit process fails.  Resource ownership should therefore be released in \texttt{free}, keyed off state that the other callbacks record in \texttt{data}, rather than in \texttt{commit} or \texttt{rollback} themselves.  A free handler is told whether the runtime system expects to retry the current page request after rollback finishes.  The return value of \texttt{uw\_register\_transactional()} is 0 on success and nonzero on failure (where failure currently only happens when exceeding configured limits on number of transactionals).
