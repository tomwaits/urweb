...
\item \texttt{-ccopt FLAGS}: Optimization flags passed to the C compiler for the generated code.  The default is \texttt{-O3}.  Because the whole application is emitted as a single large C file, \texttt{gcc -O3} dominates compile time for big applications, so lowering this (e.g. \texttt{-ccopt -O0} or \texttt{-ccopt -O1}) makes iterative and CI builds much faster at the cost of runtime performance; keep \texttt{-O3} for production.  (\texttt{-debug} forces no optimization regardless.)
