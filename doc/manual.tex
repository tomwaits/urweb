...
\item \texttt{notFoundPage HTML} sets a custom body for the built-in 404 \texttt{Not Found} page, served (as \texttt{text/html}) for any URL that matches no page handler.  The argument is the rest of the line, taken verbatim as the HTML body; the default is a plain-text \texttt{Not Found}.  This styles the one error page that is not reachable through \texttt{onError} (which handles fatal 500s).
